...
def preprocess_all():
    try:
        ...
        match dataset_selection:
            ...
            case 2:
                ...
                dataset = 'Seven_Phishing_Email_Datasets'
            case 3:
    ...
    preprocessed_folder = os.path.join(os.path.dirname(os.path.dirname(data_folder)), 'preprocessed')
 ...
 # Ensure label exists
        ...
        for col in df.columns:
            if df[col].dtype == object:
                if df[col].isna().all() or df[col].str.strip().eq('').all():
                    print(f"Column '{col}' is empty, replacing with empty string.")
                    df[col] = ""
    ...
    df_final = df[['clean_email_text', 'label']].rename(columns={'clean_email_text': 'text'})
